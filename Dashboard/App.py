...
.metric-card {
    ...
    box-shadow: 0 1px 6px rgba(0,0,0,0.09); text-align: center;
    min-height: 110px; display: flex; flex-direction: column; justify-content: center;
}
.metric-card .label {
    font-size: 0.68rem; letter-spacing: .08em; color: #64748b;
    text-transform: uppercase; margin-bottom: 6px; line-height: 1.4;
}
.metric-card .value {
    font-size: 1.15rem; font-weight: 800; color: #0f172a; line-height: 1.25;
    word-break: break-word; overflow-wrap: break-word; white-space: normal;
}
.metric-card .sub { font-size: 0.74rem; color: #94a3b8; margin-top: 5px; }
...
.section-sub { font-size: 0.82rem; color: #94a3b8; margin-bottom: 18px; }

.insight-box {
    background: #eff6ff; border-left: 4px solid #3b82f6; border-radius: 0 10px 10px 0;
    padding: 12px 16px; font-size: 0.88rem; color: #1e40af; margin-top: 12px; line-height: 1.6;
}
...
KURS_USD_IDR = 17_863
RASIO = {
    'pendidikan': 0.20, 'tempat_tinggal': 0.30, 'makanan': 0.35,
    'transportasi': 0.30, 'buku': 0.40, 'hiburan': 0.30,
    'perawatan': 0.35, 'teknologi': 0.45, 'kesehatan': 0.35, 'lainnya': 0.35,
}
STATUS_ORDER  = ["Bahaya", "Waspada", "Stabil", "Sangat Sehat"]
STATUS_COLORS = {"Bahaya":"#ef4444","Waspada":"#f59e0b","Stabil":"#3b82f6","Sangat Sehat":"#22c55e"}
CLUSTER_PALETTE = ["#3b82f6","#f59e0b","#22c55e","#ef4444","#8b5cf6","#06b6d4"]
LABEL_KOLOM = {
    'total_pemasukan':'Total Pemasukan','total_pengeluaran':'Total Pengeluaran',
    'sisa_uang':'Sisa Uang','rasio_pengeluaran':'Rasio Pengeluaran',
    'pendidikan':'Pendidikan','tempat_tinggal':'Tempat Tinggal','makanan':'Makanan',
    'transportasi':'Transportasi','buku':'Buku & Alat Tulis','hiburan':'Hiburan',
    'perawatan':'Perawatan Diri','teknologi':'Teknologi','kesehatan':'Kesehatan',
    'lainnya':'Lainnya','financial_score':'Skor Keuangan',
    'usia':'Usia','pendapatan':'Pendapatan','bantuan':'Bantuan Keuangan',
}
...
def fmt_rupiah(val, short=True):
    """Selalu gunakan Jt (bukan M) untuk nilai besar."""
...
# FIX: margin TIDAK ada di PLOTLY_LAYOUT — tiap chart set margin sendiri
# agar tidak ada konflik key saat update_layout menerima margin=dict(...)
PL = dict(
    paper_bgcolor="rgba(0,0,0,0)",
    plot_bgcolor="rgba(0,0,0,0)",
    font=dict(family="Plus Jakarta Sans, sans-serif", color="#334155"),
)
M  = dict(t=40, b=30, l=10, r=10)   # margin default
ML = dict(t=40, b=20, l=160, r=10)  # margin heatmap (y-label lebar)
...
def load_and_process_data(file_key: str):
    ...
    df = df_raw.rename(columns={
        'Unnamed: 0':'id','age':'usia','monthly_income':'pendapatan',
        'financial_aid':'bantuan','tuition':'pendidikan','housing':'tempat_tinggal',
        'food':'makanan','transportation':'transportasi','books_supplies':'buku',
        'entertainment':'hiburan','personal_care':'perawatan','technology':'teknologi',
        'health_wellness':'kesehatan','miscellaneous':'lainnya',
    })
    ...
    def hitung_score(row):
        ti = row['total_pemasukan']; tp = row['total_pengeluaran']
    ...
    cat_cols = [c for c in df.select_dtypes('object').columns if c not in ['status_finansial','nama_cluster']]
    df_enc = df.copy()
    for col in cat_cols:
        le = LabelEncoder()
    ...
    fitur = (
        ['total_pemasukan','total_pengeluaran','sisa_uang','rasio_pengeluaran',
         'pendidikan','tempat_tinggal','makanan','transportasi','hiburan','teknologi','financial_score']
    ...
    kol = [c for c in ['total_pemasukan','total_pengeluaran','sisa_uang','rasio_pengeluaran','financial_score'] if c in df.columns]
    ...
    meta = dict(
        ...
        fitur=list(fitur), sil_scores={int(k):float(v) for k,v in sil_scores.items()},
...
with st.sidebar:
    ...
    cluster_filter = st.multiselect("Cluster", options=df['nama_cluster'].dropna().unique().tolist(),
...
with tab1:
    ...
    with col_right:
        ...
        fig = px.bar(x=dist_status.values, y=dist_status.index, orientation='h',
                     color=dist_status.index, color_discrete_map=STATUS_COLORS,
...
with tab2:
    ...
    eda_sub = st.selectbox("Pilih analisis:", [
        "Distribusi Numerik","Boxplot Pengeluaran","Komposisi Pengeluaran",
        "Matriks Korelasi","Distribusi Kategorikal"
    ])

    if eda_sub == "Distribusi Numerik":
        numerik_cols   = [c for c in ['usia','pendapatan','bantuan','total_pemasukan',
                          'total_pengeluaran','sisa_uang','rasio_pengeluaran','financial_score'] if c in fdf.columns]
        numerik_labels = [kolom_label(c) for c in numerik_cols]
        fig = make_subplots(rows=2, cols=4, subplot_titles=numerik_labels,
                            vertical_spacing=0.15, horizontal_spacing=0.08)
        for i, col_n in enumerate(numerik_cols):
            row_n, col_idx = divmod(i, 4)
            data_col = fdf[col_n].dropna()
            fig.add_trace(go.Histogram(x=data_col, nbinsx=25, name=kolom_label(col_n),
        ...
        for i, col in enumerate(pengeluaran_cols):
            fig.add_trace(go.Box(y=fdf[col].dropna(), name=kolom_label(col),
                                 marker_color=px.colors.qualitative.Pastel[i % 10], boxmean=True))
        fig.update_layout(**PL, height=420, margin=M,
                          title="Boxplot Pengeluaran per Kategori (IDR)",
                          yaxis_title="IDR", showlegend=False)
        st.plotly_chart(fig, use_container_width=True)

    elif eda_sub == "Komposisi Pengeluaran":
        rata = fdf[pengeluaran_cols].mean().sort_values(ascending=False)
        rata.index = [kolom_label(c) for c in rata.index]
        ...
        with col_l:
            ...
            st.plotly_chart(fig, use_container_width=True)
        with col_r:
            fig = px.bar(x=rata.values[::-1], y=rata.index[::-1], orientation='h',
            ...
            fig.update_traces(textposition='outside')
            fig.update_layout(**PL, height=380, coloraxis_showscale=False, showlegend=False, margin=M)
            st.plotly_chart(fig, use_container_width=True)
        ...
        fig = px.imshow(corr_masked, text_auto=".2f", color_continuous_scale='RdYlGn',
                        zmin=-1, zmax=1, title="Matriks Korelasi Antar Variabel", aspect="auto")
        fig.update_layout(**PL, height=560, margin=M)
        st.plotly_chart(fig, use_container_width=True)
        ...
        if cat_cols_show:
            ...
            fig.update_traces(textposition='outside')
            fig.update_layout(**PL, height=360, coloraxis_showscale=False, margin=M)
            st.plotly_chart(fig, use_container_width=True)
        else:
...
with tab3:
    st.markdown('<div class="section-card"><div class="section-title">🤖 Proses Clustering K-Means</div>'
                '<div class="section-sub">Seleksi cluster optimal, visualisasi PCA, dan metrik evaluasi</div>', unsafe_allow_html=True)
    ...
    with col_b:
        ev_data = {
            "Metrik":     ["Jumlah Cluster (k)","Silhouette Score","Davies-Bouldin Index","Total Data","Fitur"],
            "Nilai":      [str(best_k), f"{meta['sil']:.4f}", f"{meta['db']:.4f}", str(len(df)), str(len(meta['fitur']))],
            "Keterangan": ["Segmen mahasiswa","→1 makin baik","→0 makin baik","Data valid (non-null)","Dimensi input"],
        }
    ...
    fig = px.scatter(pca_df_plot, x='PC1', y='PC2', color='nama_cluster',
                     ...
                     labels={'PC1':f"PC1 ({meta['var_exp'][0]:.1f}%)",
                             'PC2':f"PC2 ({meta['var_exp'][1]:.1f}%)",'nama_cluster':'Cluster'})
    c_df = meta['centroid_df'].copy()
    c_df['label'] = c_df['cluster'].map(nama_cluster)
    fig.add_trace(go.Scatter(x=c_df['PC1'], y=c_df['PC2'], mode='markers',
    ...
    fig.update_layout(**PL, height=440, margin=dict(t=40, b=80, l=10, r=10),
                      legend=dict(orientation='h', yanchor='bottom', y=-0.28, xanchor='center', x=0.5))
    st.plotly_chart(fig, use_container_width=True)

    st.markdown(f"""<div class="insight-box">
    💡 <b>Tentang PCA:</b> PC1 + PC2 menjelaskan <b>{meta['var_exp'][0]+meta['var_exp'][1]:.1f}%</b> total variasi.
...
with tab4:
    st.markdown('<div class="section-card"><div class="section-title">🏷️ Profil & Karakteristik Cluster</div>'
                '<div class="section-sub">Ringkasan mendalam setiap segmen mahasiswa</div>', unsafe_allow_html=True)
    ...
    for col, cl in zip(cols_cards, cluster_list):
        ...
        with col:
            st.markdown(f"""
            <div style="background:white;border-top:4px solid {color};border-radius:10px;
                ...
                <div style="font-size:14px;font-weight:800;color:{color};margin:6px 0;line-height:1.3;">
    ...
    kolom_hm = [c for c in ['total_pemasukan','total_pengeluaran','sisa_uang',
                             ...
                             'makanan','hiburan','teknologi','financial_score'] if c in profil.columns]
    ...
    def _fmt_cell(v, col):
        if col == 'rasio_pengeluaran': return f"{v:.2f}"
        if col == 'financial_score':   return f"{v:.1f}"
        return fmt_rupiah(v)

    fig = go.Figure(go.Heatmap(
        z=profil_norm.values.T, x=x_labels, y=[kolom_label(c) for c in kolom_hm],
        ...
        colorbar=dict(title="Z-score"), hoverongaps=False,
    ))
    fig.update_layout(**PL, height=420, margin=ML,
    ...
    kpi_cols = [c for c in ['total_pemasukan','total_pengeluaran','sisa_uang',
                             'rasio_pengeluaran','hiburan','teknologi','financial_score'] if c in fdf.columns]
    ...
    fig = px.bar(kpi_vals, x='Cluster', y='Nilai', color='Cluster',
                 ...
                 text=kpi_vals['Nilai'].apply(
                     lambda v: f"{v:.2f}" if selected_kpi in ['rasio_pengeluaran','financial_score'] else fmt_rupiah(v)
                 ),
                 title=f"Rata-rata {kolom_label(selected_kpi)} per Cluster",
                 labels={'Nilai': kolom_label(selected_kpi)})
    fig.update_traces(textposition='outside')
    fig.update_layout(**PL, height=360, showlegend=False, margin=M,
    ...
    fig.update_layout(**PL, height=380, margin=dict(t=40, b=80, l=10, r=10),
                      xaxis=dict(tickangle=0, gridcolor='#e2e8f0'),
    ...
    fig.update_layout(**PL, height=400, showlegend=False, margin=M,
                      xaxis=dict(tickangle=0, gridcolor='#e2e8f0'))
    st.plotly_chart(fig, use_container_width=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
st.markdown(
    ...
    f'Financial Clustering Dashboard · K-Means (k={best_k}) · Data Mahasiswa IDR</div>',
    unsafe_allow_html=True)
